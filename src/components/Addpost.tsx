...
interface user {
  ...
  function addpost(){
   if(post.current){
    ...
    axios.post('https://social-backend-navy.vercel.app/user/post',{description:data},{headers:headers}).then(()=>{alert('post updated successfully')})
   }
  }
  ...
  return (
    <div className='bg-gray-200 h-20 rounded-xl flex items-center justify-evenly p-4'>
      <div className=''>
        <input type="text" ref={post} placeholder='Share something with the world' className='h-12 sm:w-96  rounded-lg p-2' />
      </div>
      <div className="" >
